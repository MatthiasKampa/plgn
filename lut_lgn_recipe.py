"""
"Winning-recipe" candidate for chip-native deep-LGN training. Combines four fixes,
each targeting a diagnosed failure mode of the earlier attempts:

  1. GLOROT/fan-in INIT  -> sigma_e = GAIN*sqrt(2/(deg_i+deg_j)); every node's field is
     O(1), so the latent gate-outputs oA,oB are DRIVEN by their inputs from step 0
     (the old sigma=0.2 init left them decoupled -> no signal -> stuck at chance).
  2. TEMPERATURE CURRICULUM -> beta ramps 0.4 -> 1.6 over the first 70% of steps then
     holds. Hot start keeps the negative chain MIXING while structure forms (the lesson
     of Extropic's ACP = Adaptive Correlation Penalty); gradual cooling sharpens.
  3. MEAN-FIELD POSITIVE PHASE -> low-variance data statistics (the DBM recipe);
     short Gibbs sampling through latents was too noisy.
  4. PCD NEGATIVE PHASE on THRML -> persistent fantasy particles (the chip samples).
+ weight decay (keep mixing), best-checkpoint.  Still no off-chip backprop.
...
sigma_e = GAIN * jnp.sqrt(2.0 / (deg_arr[EI] + deg_arr[EJ]))   # Glorot / fan-in init
weights = sigma_e * jax.random.normal(kw, (N_EDGES,))
biases = jnp.zeros(N)                                          # centered +/-1 units
BASE = IsingEBM(NODES, EDGES, biases, weights, jnp.array(1.0))
def rebuild(b, w, beta): return eqx.tree_at(lambda e: (e.biases, e.weights, e.beta), BASE, (b, w, jnp.array(beta)))
...
@jax.jit
def mf_positive(b, w, beta, n_iter=25):                        # mean-field data statistics at temperature beta
...
def eval_acc(b, w, key, beta_eval=3.0):
    ebm = rebuild(b, w, beta_eval); prog = IsingSamplingProgram(ebm, NEG_FREE, NEG_CLAMP)
...
# temperature curriculum: hot -> cool, then hold
STEPS, WD = 500, 1e-3
B0, B1, RAMP = 0.4, 1.6, 0.7
def beta_at(step): return B0 + (B1 - B0) * min(1.0, (step / STEPS) / RAMP)

print(f"\nrecipe: Glorot init (gain {GAIN}) + temp curriculum (beta {B0}->{B1}) + MF-positive + PCD-negative")
b, w = biases, weights
mb = jnp.zeros(N); vb = jnp.zeros(N); mw = jnp.zeros(N_EDGES); vw = jnp.zeros(N_EDGES)
neg_state = hinton_init(key, BASE, NEG_FREE, (P,))
neg_state = persist(sample_phase(IsingSamplingProgram(rebuild(b, w, B0), NEG_FREE, NEG_CLAMP), SCHED_BURN, jax.random.split(key, P), neg_state, NEG_CB, NEG_OBS), 3)
print(f"{'step':>5}{'beta':>6}{'acc':>8}{'best':>7}{'|w|':>7}")
best_acc, best, traj = -1.0, (b, w), []
t0 = time.time()
for step in range(1, STEPS + 1):
    beta_t = beta_at(step)
    key, kn, ke = jax.random.split(key, 3)
    sb_p, ee_p = mf_positive(b, w, beta_t)
    sn = sample_phase(IsingSamplingProgram(rebuild(b, w, beta_t), NEG_FREE, NEG_CLAMP), SCHED_STEP, jax.random.split(kn, P), neg_state, NEG_CB, NEG_OBS)
    neg_state = persist(sn, 3); sb_n, ee_n = neg_moments(assemble(sn, NEG_OBS_IDX, I_IN, X))
    gb = -(sb_p - sb_n) + WD * b; gw = -(ee_p - ee_n) + WD * w
    lr = 0.005 + 0.015 * (1.0 - step / STEPS)
    ...
    if step % 20 == 0 or step == 1:
        ...
        print(f"{step:>5}{beta_t:>6.2f}{int(round(acc*P)):>5}/{P}{int(round(best_acc*P)):>5}/{P}{float(jnp.linalg.norm(w)):>7.2f}", flush=True)
print(f"trained in {time.time()-t0:.0f}s ; BEST = {int(round(best_acc*P))}/{P}")
b, w = best; key, ke = jax.random.split(key)
print(f"\ntrajectory(/64): {traj}")
print(f"RECIPE best-checkpoint: {int(round(eval_acc(b, w, ke)*P))}/{P}   "
      f"(prior chip-native attempts all near chance ~33-44/64)")
